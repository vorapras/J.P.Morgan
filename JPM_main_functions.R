#####################################################################################
## These are the functions needed in order to run script "JPM_main.R"
#####################################################################################
...
## Predicted Values from the ARIMA model
ARIMA_predict2 = function(coef,test_real,X_test,p){
  
  # Data Matrix Transformation
  Xlag = matrix(0, nrow=4, ncol=1)
  #Residlag = as.matrix(resid)
  Residlag = matrix(0, nrow=4, ncol=1)
  new_X = matrix(0,nrow=9,ncol=1)
  
  Diff_X = matrix(NA, nrow=length(test_real), ncol=4)
  
  for (j in 1:(length(test_real)-1)){
    
    for (z in 1:p){
      
      for (i in 1:p){
        Xlag[i] = X_test[i+j]-X_test[i+j-1]
        
        if ((j>z)&(z>=i)){
          Residlag[i] = Diff_X[j-i,z] - test_real[j-i]
        }
      }
      
      new_X = rbind(Xlag,Residlag,1)
      Diff_X[j,z] = coef[z,]%*%new_X
    }
  }  
  
  lagX1 = lag_1(X_test)
  lagX1 = lagX1[2:length(lagX1)]
  lagX = cbind(lagX1,lagX1,lagX1,lagX1)
  fitdata = Diff_X+lagX
  return (fitdata)
}
...
ecm_model = function(Y,X,numlag){
  ...
  print(summary(reg.ols))
  qqnorm(resid.ols)
  
  # test that error term is stationary
  testunit = ur.df(resid.ols, type="none", lags=1, selectlags = "AIC")
  for(i in 1:3){
    if( abs(testunit@teststat) > abs(testunit@cval[i]) ) {print("It is staionary.")}
    else {print("It is nonstaionary.")}
  }
  ...
  for(i in 2:(numlag+1)){
    ...
    print(summary(reg_ecm))
    qqnorm(residuals(reg_ecm))
...
test_forecast = function(fit_data,actual_data,data_for_spread,data_for_profit,timestamp){
  ...
  hist(profit_ar, breaks=20, col="red", xlab="Profit (Won)", main="Histogram of profit")
  
  # Plot Prediction versus actual data
  plot(timestamp, actual_data, type='l', ylab="Price (Won)", xlab="Timestamp", col="red", main="Plot of predicted prices")
  lines(timestamp, fit_data, col="green")
  legend("topright",c("Actual price","Fitted price"), fill=c("red","green"), cex=0.5)
