#####################################################################################
## JPM_validate builds models and cross-validates each model on 80% of the data
#####################################################################################
...
# JPM_validate_functions.R contains all functions needed for this script
source("/Users/ZerWinner/Desktop/JPMorgan Challenge/JPM_validate_functions.R")
...
# Create list to store results from testing
...
# Loop through the first 80% of the data to train and validate all 84 models on each day
for (z in 1:(validation_end-1)){
...
for (i in c("spot_bid", "spot_ask", "future_bid", "future_ask")){
  ...
  lags=c(1,2,3,4)
  plot(lags, AR_results$MSE, type="l", col="red", xlab="Lags", ylab="MSE", main=paste(i," model MSE",sep=""), xaxt='n', ylim=c(ymin,ymax))
  axis(1, at=1:4)
...
for (i in c("spot", "future")){
  ...
  ymin = min(plot_limits)
  lags=c(1,2,3,4)
  plot(lags, AR_results$profit, type="l", col="red", xlab="Lags", ylab="Profit (USD) per trading period", main=paste(i," model profit",sep=""), xaxt='n', ylim=c(ymin,ymax))
  axis(1, at=1:4)
  ...
  lines(lags, Dummy_plot, col="black", lty=2)
  legend("bottomleft",c("AR","ARIMA", "VAR", "ECM","VECM", "Dummy"), fill=c("red","cyan","green", "orange","blue","black"), cex=0.5, lty=c(1,1,1,1,1,2))
}